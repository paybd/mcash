...
export default function RatesCalculator() {
  ...
  const competitorTapTap = useMemo(() => {
    return receiveAmount * 0.93; // 7% less than Mcash Remit
  }, [receiveAmount]);

  const competitorRia = useMemo(() => {
    return receiveAmount * 0.92; // 8% less than Mcash Remit
  }, [receiveAmount]);
      ...
      {tab === "calculator" ? (
        <div className="mt-4 space-y-3 text-sm">
          ...
          <div className="mt-2 text-center text-base text-white/70">
            আমাদের ব্যবসায়িক রেট চান? <a className="underline font-semibold" href="https://wa.me/?text=আমি Mcash Remit সেলস টিমের সাথে যোগাযোগ করতে চাই" target="_blank" rel="noopener noreferrer">সেলস টিমের সাথে যোগাযোগ করুন</a>।
          </div>
        </div>
      ) : (
        <div className="mt-4 space-y-3 text-sm">
          {[
            { name: "Mcash Remit", value: receiveAmount, highlight: true },
